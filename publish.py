...
def git_commit(message):
    ...
    try:
        git("push", "-u", "origin", "claude/optimistic-darwin-ut50m4")
        print("  ↳ git push ok")
...
def main():
    ...
    ap.add_argument("--no-db", action="store_true", help="skip DB seed (use when Postgres port is blocked)")
    ...
    inserted = []
    if args.no_db:
        print("  ! --no-db set — skipping DB seed", file=sys.stderr)
    elif not dsn:
