...
Two DB modes (auto-detected):
  A. DATABASE_URL set → psycopg direct connection (schema auto-discovered)
  B. SUPABASE_URL + SUPABASE_SERVICE_KEY set → Supabase REST API (schema from OpenAPI)

Run `python publish.py --probe` first to print the discovered mapping (no writes).

Env (put in .env or export):
  DATABASE_URL            Postgres connection string (mode A)
  SUPABASE_URL            e.g. https://xyz.supabase.co (mode B)
  SUPABASE_SERVICE_KEY    service role JWT (mode B)
  ...
  CONTENT_SLUG            (optional) project slug, default "date-khaas"
"""
...
# ----------------------------------------------------------------------------- #
# schema discovery — psycopg mode
# ----------------------------------------------------------------------------- #

def discover_schema_psycopg(cur):
    cur.execute("""
...
# ----------------------------------------------------------------------------- #
# schema discovery — Supabase REST API mode
# ----------------------------------------------------------------------------- #

def discover_schema_rest():
    """Discover content table schema via Supabase PostgREST OpenAPI spec."""
    ...
    if requests is None:
        raise SystemExit("`requests` not installed — cannot use REST API mode.")

    r = requests.get(f"{supabase_url}/rest/v1/", headers=headers, timeout=20)
    r.raise_for_status()
    spec = r.json()
    ...
    found_table = None
    found_type_col = override_col
    found_cols = {}

    if override_table:
        defn = definitions.get(override_table, {})
        props = defn.get("properties", {})
        for cn, cd in props.items():
            found_cols[cn] = _rest_col_info(cn, cd)
        found_table = override_table
    else:
        # Find the table whose type/enum column matches our content labels
        for def_name, definition in definitions.items():
            props = definition.get("properties", {})
            for col_name, col_def in props.items():
                enum_vals = col_def.get("enum", [])
                if enum_vals and set(enum_vals) & TARGET_ENUM_LABELS:
                    found_table = def_name
                    found_type_col = col_name
                    for cn, cd in props.items():
                        found_cols[cn] = _rest_col_info(cn, cd)
                    break
            if found_table:
    ...
    if not found_table:
        ...
        for candidate in ["content_items", "content", "hub_content", "posts", "articles"]:
            try:
                probe = requests.get(
                    f"{supabase_url}/rest/v1/{candidate}",
                ...
                if probe.status_code < 400:
                    ...
                    # We don't know columns yet; get them from an empty select
                    if probe.status_code == 200 and probe.json():
                        for cn in probe.json()[0].keys():
                            found_cols[cn] = {"data_type": "text", "udt": "text",
                                              "nullable": True, "default": None,
                                              "is_enum": False, "is_json": False}
                    break
            except Exception:
                continue
    ...
    if not found_table:
        raise SystemExit(
            "Could not auto-discover content table via Supabase REST API.\n"
            "Set CONTENT_TABLE env var (e.g. CONTENT_TABLE=content_items)."
        )

    return found_table, found_type_col, found_cols

def _rest_col_info(col_name, col_def):
    ...
    return {
        ...
        "nullable": True,
        "default": None,
        "is_enum": bool(col_def.get("enum")),
        "is_json": typ == "object" or fmt in ("json", "jsonb"),
    }

def insert_via_rest(supabase_url, service_key, table, record):
    """Insert a single record via Supabase REST API. Returns the inserted id."""
    ...
    r = requests.post(url, json=[record], headers=headers, timeout=30)
    if not r.ok:
        raise RuntimeError(f"REST insert failed {r.status_code}: {r.text[:300]}")
    result = r.json()
    if result:
        row = result[0]
        return row.get("id") or row.get("uuid") or row.get("pk") or list(row.values())[0]
    return None

# ----------------------------------------------------------------------------- #
# insert builder (psycopg mode)
# ----------------------------------------------------------------------------- #
...
def build_rest_record(type_col, columns, item, slug, workspace, status):
    """Build a plain dict suitable for Supabase REST API insert."""
    ...
    record = {type_col: item["type"]}
    for canon, aliases in FIELD_ALIASES.items():
        col = pick(columns, aliases) if columns else None
        if not col:
            # If we don't know columns, try the first alias as a guess
            col = aliases[0]
        if col in record:
            continue
        val = canon_value.get(canon)
...
def git_commit(message):
    ...
    try:
        git("push", "-u", "origin", "claude/peaceful-turing-92q69d")
        print("  ↳ git push ok")
...
def main():
    ...
    ap.add_argument("--dry-run", action="store_true", help="build + print SQL/record, do not write")
    ...
    use_psycopg = bool(dsn and psycopg)
    use_rest = bool(supabase_url and service_key)

    if not use_psycopg and psycopg is None and dsn:
        print("  ! psycopg not installed — falling back to REST API mode", file=sys.stderr)

    # Test psycopg connectivity; fall back to REST on network failure
    if use_psycopg:
        try:
            with psycopg.connect(dsn, connect_timeout=8, prepare_threshold=None) as _c:
                pass
        except Exception as e:
            print(f"  ! psycopg connect failed ({e.__class__.__name__}) — falling back to REST API", file=sys.stderr)
            use_psycopg = False
    ...
    if args.probe:
        if use_psycopg:
            with psycopg.connect(dsn, connect_timeout=20, prepare_threshold=None) as conn, conn.cursor() as cur:
                table, type_col, columns, enum = discover_schema_psycopg(cur)
            print(f"mode          : psycopg (DATABASE_URL)")
            print(f"content table : {table}")
            print(f"type column   : {type_col}  (enum: {enum})")
        elif use_rest:
            table, type_col, columns = discover_schema_rest()
            print(f"mode          : Supabase REST API")
            print(f"content table : {table}")
        ...
        print("field mapping :")
        for canon, aliases in FIELD_ALIASES.items():
            col = pick(columns, aliases) if columns else aliases[0]
            print(f"  {canon:<11} -> {col}")
        if columns:
            print("all columns   :")
    ...
    for i, it in enumerate(items, 1):
        ...
        if it.get("body"):
            ext = "html" if "html" in it["type"] or it.get("format") in ("amp", "standard") else "txt"
    ...
    if use_psycopg:
            ...
            with conn.cursor() as cur:
                ...
                for it in items:
                    if it["type"] not in TARGET_ENUM_LABELS:
                        print(f"  ! '{it['type']}' not in enum — skipping", file=sys.stderr)
                        continue
        ...
        table, type_col, columns = discover_schema_rest()
        print(f"  ↳ REST mode: table={table}, type_col={type_col}")
        for it in items:
            if it["type"] not in TARGET_ENUM_LABELS:
                print(f"  ! '{it['type']}' not in enum — skipping", file=sys.stderr)
                continue
            record = build_rest_record(type_col, columns, it, DEFAULT_SLUG, workspace, args.status)
            if args.dry_run:
                print(f"\nDRY RUN REST record for {it['type']}:")
                print(json.dumps({k: str(v)[:80] if isinstance(v, str) else v
                                  for k, v in record.items()}, indent=2, ensure_ascii=False))
                continue
            try:
                rid = insert_via_rest(supabase_url, service_key, table, record)
                inserted.append(rid)
                print(f"  ↳ inserted {it['type']:<11} id={rid}")
            except Exception as e:
                print(f"  ! REST insert failed for {it['type']}: {e}", file=sys.stderr)

    else:
        print("  ! No DB credentials — skipping DB seed (set DATABASE_URL or SUPABASE_URL + SUPABASE_SERVICE_KEY)",
              file=sys.stderr)
    ...
    if inserted:
        ...
        print(f"\nDone. Files written. DB step {'skipped (dry-run)' if args.dry_run else 'check logs above'}.")
