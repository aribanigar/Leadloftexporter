...
USAGE
  python publish.py --discover
      Print public tables + columns for the content-item and folder tables.

  python publish.py --folder hudace batch.json
      Ensure the folder exists in HUB_WORKSPACE, then insert every item in
      batch.json. Idempotent: an item whose title already exists in the folder
      is skipped, so re-running the same day does nothing.

ENV (required)
  DATABASE_URL    Neon Postgres URL (sslmode=require)
  HUB_WORKSPACE   workspace UUID

ENV (optional overrides, only if --discover shows different names)
  HUB_ITEM_TABLE      default tries: content_items, contents, hub_items, items, posts
  HUB_FOLDER_TABLE    default tries: folders, collections, content_folders, hub_folders
  COL_WORKSPACE       default: workspace_id
  COL_FOLDER          default: folder_id
  COL_TITLE           default: title
  COL_TYPE            default: type        (enum: html_email | whatsapp | caption | sms | other)
  COL_BODY            default: body        (html for emails, text for messages)
  COL_IMAGE           default: image_url
  COL_STATUS          default: status      (value set by STATUS_VALUE)
  STATUS_VALUE        default: draft

batch.json  = a JSON list of items:
  [
    {"title": "[2026-06-18] Retail | Social Media", "type": "html_email",
     "body": "<html>...</html>", "image_url": "https://.../hero.jpg"},
    {"title": "[2026-06-18] Retail | Social Media (WhatsApp)", "type": "whatsapp",
     "body": "Reels, ads, full management ...", "image_url": "https://.../hero.jpg"}
  ]
"""
