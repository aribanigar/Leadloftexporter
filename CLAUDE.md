...
- **`get_extension_context`** — `X-API-Key` **and** `X-License-Key` (both required) for the Chrome extension. API keys are SHA-256 hashed at rest and prefix-indexed for fast lookup, generated in **Settings → API Keys**. License keys are validated alongside them — see "License keys, invites, and the single platform admin" below for what each credential actually gates.
...
### License keys, invites, and the single platform admin

Every self-registered user (`POST /auth/register`) gets their **own independent workspace** as its "owner" (`auth.py:register` → `_unique_slug`, 14-day trial) — this is a normal multi-tenant signup, not a shared team workspace. On top of that, activation is gated by a second, deliberately narrow credential system:

- **`LicenseKey`** (`models/base.py`) is a separate, admin-controlled gate from the `ApiKey` — an `ApiKey` already determines whose account captured data belongs to (`ApiKey.user_id` → `Lead.owner_id`); a `LicenseKey` doesn't touch that, it just decides whether the pair is allowed to work at all. `workspace_id` is **nullable** because a `LicenseKey` has two lifecycles in one table: issued directly into an existing owned workspace (`assigned_user_id` set immediately), or issued as an **unclaimed invite** (`workspace_id`/`assigned_user_id` both NULL, optionally pinned to one email via `invite_email`) that `auth.py:register`'s `_find_unclaimed_license` validates and claims — filling in both fields — atomically with account creation, so the exact same key value the admin handed out keeps working for the extension once registration succeeds.
- **Registering requires a valid unclaimed license key** as an invite code (`body.license_key`), except for the platform admin's own email. **Signing in also re-checks a valid license key every time** (`auth.py:_require_license_to_sign_in` — active, not expired, workspace-matched, and if `assigned_user_id` is set it must be that user) — exempting only the platform admin and any user whose *every* membership is the lightweight `campaigns_only` role (`_user_is_campaigns_only`; see Team below), since those accounts never touch the extension. This means revoking someone's license key locks them out of login itself, not just the extension.
- **API keys are self-service per account** (`workspaces.py:create_api_key`) — anyone can mint one for **themselves**; that's what connects the extension to their own workspace. Minting one for someone ELSE requires the platform admin. `delete_api_key` is scoped to the key's own owner or the admin.
- **License keys are strictly admin-only** end to end (`licenses.py` — list/create/patch/reset/delete), scoped to the admin's own current workspace.
- **`admin_access.py`** is the bridge for the case the two routers above can't reach: granting a key into *another* user's *own, unrelated* workspace. Since every self-registered user owns an independent workspace, and `create_api_key`/`licenses.py` only ever touch the **caller's own** `ctx.workspace_id`, there was no way to hand a stranger's already-registered workspace a key without first joining it as a Membership — which would also grant read access to their leads/campaigns. `admin_access.py` avoids that entirely: every endpoint uses `get_current_user` (JWT only, no `get_workspace_context`), looks up the target's owned workspace by email (`_find_owned_workspace` — the workspace where they hold role `"owner"`), and mints directly into it with **no Membership row created for the admin**. Also hosts `/invite` (issue an unclaimed license key for an email with no account yet, per the invite lifecycle above) and `/lookup` (confirm who a key is about to go to before issuing).
- **This is a separate mechanism from Team** (`team.py`, `ASSIGNABLE_ROLES = {"admin","member","campaigns_only"}`): Team adds someone **directly into the caller's own workspace** with a Membership and a role, sharing that workspace's data immediately — for actual teammates. `admin_access.py` grants touch only the credential, never create a Membership, and never expose the target workspace's contents — for administering access to *other people's separate* workspaces.
- **The admin's email is one literal string, copy-pasted on purpose in four places**: `workspaces.py:LICENSED_API_KEY_ADMIN_EMAIL`, `licenses.py:LICENSE_KEY_ISSUER_EMAIL`, `admin_access.py:PLATFORM_ADMIN_EMAIL`, `auth.py:PLATFORM_ADMIN_EMAIL` — each currently `"acemedia.qa@gmail.com"`. Every file's own comment explains why it's not imported from one shared constant: no import-order coupling between these routers. **If this account ever changes, all four literals must be updated together** — grep for the value, not a symbol name.
