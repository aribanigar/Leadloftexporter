...
def ingest_lead(
    ...
    if existing:
        ...
        # were fill-only, which is why some leads showed in the CRM without
        # a location even after the extension's panel said 'Saved ✓
        # (location)' — the row already had a stale value from the initial
        # card-level save and the new accurate one was silently dropped.
        ...
        # Company — OVERWRITE when the fresh scrape resolved one. Was
        # fill-only, which left rows pinned to the wrong company when
        # the original card-level save matched a generic name.
        if company:
