...
# Always allow the extension (chrome-extension://*), any Vercel deployment of
# the web app (production + preview URLs), and the production custom domain
# (hudace.com + subdomains) regardless of what FRONTEND_ORIGINS is set to on a
# given backend service. This makes the frontend's automatic backend-failover
# robust: whichever Render service the browser lands on will accept the real
# origin even if that service's env wasn't configured with it — a hand-created
# Render service inherits none of the Blueprint's env vars, so this has bitten
# more than once as the backend moved between services. Regex-matched origins
# are echoed back (not "*"), so this stays compatible with allow_credentials=True.
app.add_middleware(
    CORSMiddleware,
    allow_origins=_settings.cors_origins or ["*"],
    allow_origin_regex=r"(chrome-extension://.*|https://([a-z0-9-]+\.)*vercel\.app|https://([a-z0-9-]+\.)*hudace\.com)",
