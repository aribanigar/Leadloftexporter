...
Each set: email.html  email.amp.html  whatsapp.txt  linkedin.txt  meta.json
          img/whatsapp.jpg  ->  content/via-kashmir-itinerary/<date>/<market>/ .
...
WA = "https://wa.me/918218929990?text=Hi%20ViaKashmir%2C%20I%27d%20like%20to%20try%20the%20itinerary%20builder"
CONTACT = "contact@viakashmir.in &nbsp;&nbsp;|&nbsp;&nbsp; +91 82189 29990"
...
def amp_email_html(cfg):
    """AMP4Email version - same structure, amp-img tags."""
    body = email_html(cfg, preview=False)
    # Convert to AMP: replace DOCTYPE + html tag, add AMP boilerplate, convert img to amp-img
    body = body.replace('<!DOCTYPE html>', '<!doctype html>')
    body = body.replace('<html lang="en" xmlns="http://www.w3.org/1999/xhtml">',
                        '<html ⚡4email lang="en">')
    ...
    body = _re.sub(
        r'<img ([^>]+)style="([^"]*)"([^>]*)>',
        lambda m: f'<amp-img {m.group(1)}style="{m.group(2)}"{m.group(3)} layout="responsive"></amp-img>',
        body
    )
    return body
...
<div class="wrap">
  ...
  <div class="contact">contact@viakashmir.in &nbsp;|&nbsp; +91 82189 29990</div>
</div></body></html>"""
...
def write_set(date, market, preview):
    ...
    (out / "email.amp.html").write_text(amp_email_html(cfg), encoding="utf-8")
    ...
    ch = out / "img" / "_card.html"
    ch.write_text(card_html(cfg), encoding="utf-8")
    try:
