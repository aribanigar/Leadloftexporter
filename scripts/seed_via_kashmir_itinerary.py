...
LOGO  = "https://viakashmir.in/logo-colour.svg?v=3"   # no itinerary-specific logo exists
...
class DB:
    """Thin psycopg2 wrapper. .q() keeps the $1,$2 placeholder style this script already
    uses and maps it to psycopg2's %s (reordering params to match order of appearance)."""
...
def main() -> int:
    ...
    for sd in sorted({p.parent for p in day.rglob("meta.json")}):
        ...
        subj = m.get("subject")
        track = m.get("track", sd.name)          # = market (india|kashmir|saudi|dubai)
